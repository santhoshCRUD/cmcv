...
${data.postLogin?`
<!-- ============ Mobile topbar (<lg) ============
     Logo + hamburger only. The hamburger opens the SAME #navbarText
     offcanvas that already existed here (untouched below) - that drawer
     already was this app's mobile nav pattern, so it's reused as-is. -->
<div class="app-topbar d-flex d-lg-none align-items-center justify-content-between bg-primarycolor">
  <div class="d-flex align-items-center">
    <img src="/images/cmc-logo-white-transparent.png" class="img-fluid" width="40px" />
    <span class="d-flex align-items-center text-white text-center fw-bold trirong-bold ms-1"><span>CMC</span><span
        class="primarycolor fs-3">V</span><span>CONNECT</span></span>
  </div>
  <button class="btn app-topbar-toggle" data-bs-toggle="offcanvas" data-bs-target="#navbarText"
...
<!-- ============ Desktop sidebar (>=lg) ============
     Persistent left navigation - same nav items, same onclick handlers,
     same role-gated data variables as before (see the mentorBtn /
     menteeBtn / etc. context built in renderPostHomePage(), app.js) -
     just laid out as a vertical column instead of a horizontal navbar
     (RULE 9). -->
<aside class="app-sidebar navbar d-none d-lg-flex flex-column bg-primarycolor">
  <div class="app-sidebar-brand d-flex align-items-center text-white trirong-bold">
    <img src="/images/cmc-logo-white-transparent.png" class="img-fluid" width="40px" />
    &nbsp;CMC<span class="primarycolor" style="font-size:30px; line-height: 1;">V</span>CONNECT
  </div>
  ...
  <div class="app-sidebar-foot">
    ...
    <div class="dropdown app-sidebar-profile">
      <a class="nav-link dropdown-toggle d-flex align-items-center text-white" type="button" data-toggle="dropdown"
        data-bs-toggle="dropdown" aria-expanded="false">
        <img class="profile_icon rounded-circle me-2" src="${data.userImg}" alt="" style="width: 32px; height: 32px;">
        <span class="text-truncate small">${data.email}</span>
      </a>
      <div class="dropdown-menu mb-2 p-3" style="width: max-content;">
      ...
      <button class="btn btn-light btn-sm w-100 mt-2" type="button" id="${data.email}:${data.loginType}"
...
<div class="position-relative text-white eqp-page" style="min-height: 100vh; width: 100%;">
      ...
      <div class="eqp-partition eqp-partition-available">
        <div class="section-head">
          ...
          <div class="section-head-actions">
            <button type="button" class="btn btn-outline" id="dashboard-btn" hidden>
  ...
  <!-- ============ VIEW: ADMIN DASHBOARD ============
       Missions/admin role only (see dashboard-btn's hidden toggle and
       showRequestsCol in formLoad.js). Read-only KPI + activity summary
       across every request currently in the local requests store, plus
       an editable asset manager (units left, per-request cap) for every
       item ever added. switchEqpView()/renderDashboard() in formLoad.js
       drive this section; it shares the page's own .eqp-dash-* styles
       (see styles.scss, appended after the existing Asset styles). -->
  <section class="view" id="view-dashboard" hidden>
    <div class="wrap">
      <div class="section-head">
        <h2 class="partition-heading" style="color:var(--navy);">Admin Dashboard</h2>
        <button type="button" class="btn btn-outline" id="dashboard-back-btn" style="margin-left:auto;">
          <svg class="icon" viewBox="0 0 24 24"><path d="M15 6l-6 6 6 6"/></svg>
          Back to Equipment
        </button>
      </div>
      <p class="eqp-dash-note" style="margin:-8px 0 20px;">A consolidated view of every request and every listed asset. Counts reflect this browser's request data.</p>

      <div class="eqp-dash-kpis" id="eqp-dash-kpis"></div>

      <div class="eqp-dash-section">
        <h3>Activity by Hospital</h3>
        <div class="eqp-dash-table-wrap">
          <table class="eqp-dash-table">
            <thead>
              <tr><th>Hospital</th><th>Requests</th><th>Approved</th><th>Pending</th><th>Rejected</th><th>Last activity</th></tr>
            </thead>
            <tbody id="eqp-dash-hospitals"></tbody>
          </table>
        </div>
        <p class="eqp-dash-note" id="eqp-dash-hospitals-note" hidden>Requests aren't reliably tagged with a hospital yet, so activity currently groups under "Not specified" until that's added to the request form.</p>
      </div>
      ...
      <div class="eqp-dash-section">
        ...
        <div class="eqp-dash-table-wrap">
          <table class="eqp-dash-table">
            <thead>
              <tr><th>Item</th><th>Category</th><th>Units left</th><th>Max per request</th><th>Status</th><th>Requests</th></tr>
            </thead>
            <tbody id="eqp-dash-assets"></tbody>
          </table>
        </div>
        <p class="eqp-dash-note">Set "Max per request" to cap how many units a hospital can ask for in one application. Leave blank for no limit. Changes save as soon as you leave the box.</p>
      </div>
    </div>
  </section>
    ...
    <div class="drawer-columns">
      ...
      <div class="drawer-main">
        ...
        <div class="drawer-status-block" id="drawer-status-block" hidden>
          ...
          <!-- ============ Admin decision (Missions/admin side only) ============
               Only shown when viewing this request from the Requests
               (connect) column and its status is still Pending/Under
               Review — see openRequestDetail() and setRequestStatus() in
               formLoad.js. Approving reduces the asset's units by the
               quantity requested; rejecting does not touch stock. -->
          <div class="drawer-admin-actions" id="drawer-admin-actions" hidden>
            <button type="button" class="btn btn-outline" id="drawer-reject-btn">Reject</button>
            <button type="button" class="btn btn-gold" id="drawer-approve-btn">Approve</button>
          </div>
        ...
        <form id="apply-form">
          ...
          <div class="field">
            ...
            <p class="field-hint" id="apply-qty-hint" hidden></p>
            <p class="field-error" id="apply-qty-error" hidden></p>
          </div>
          <div class="field">
            <label>Could you tell us a little about why this would help?</label>
...
<div class="drawer eqp-page" id="asset-form-drawer" role="dialog" aria-modal="true" aria-labelledby="asset-form-title">
  ...
  <div class="drawer-body">
    <form id="asset-form">
      ...
      <div class="field">
        <label>Max units per request <span class="optional-tag">(optional)</span></label>
        <input type="number" min="1" id="asset-max-per-request" placeholder="e.g. 2 — leave blank for no limit">
        <p class="field-hint">Caps how many units one hospital can ask for in a single request. Doesn't limit how many units are in stock.</p>
      </div>
      <div class="field">
